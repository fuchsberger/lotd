defmodule LotdWeb.Components.Link do
  ...
  defp custom_link(%{link_type: "a"} = assigns) do
    ~H"""
  ...
  defp custom_link(%{link_type: "phx-button"} = assigns) do
    ~H"""
    <%= Phoenix.HTML.Link.button [to: @to, class: @class] ++ @extra_assigns, do: (if @inner_block, do: render_slot(@inner_block), else: @label) %>
    """
  end
