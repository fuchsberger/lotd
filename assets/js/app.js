...
var characterTable = $('#character-table').DataTable({
  ...
  columnDefs: [
    ...
    { targets: [0, 3, 4, 5, 6], type: "html" },
    { targets: 1, className: "font-medium text-gray-900 truncate"},
    { targets: 2, className: "pr-12 text-right"},
    {
      targets: 3,
      className: "hidden md:table-cell",
      render: date => `<time class="hidden">${date}</time>`
    },
    {
      targets: 4,
...
$("#delete-btn").on("click", e => {
  ...
  .done(({deleted_id}) => {
    characterTable.rows(`#entry-${deleted_id}`).remove().draw()
